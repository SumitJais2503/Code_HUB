...
const Repo = ({repo}) => {
	...
	const handleCloneClick = async(repo) => {
		// asynchronous since navigator functions used
		try {
			await navigator.clipboard.writeText(repo.clone_url);
		toast.success("Repo url cloned to clipboard!");
		} catch(error) {
			toast.error("Clipboard clone failed!");
		}
	}
	...
	return (
		<li className='mb-10 ms-7'>
			...
			<p className='mb-4 text-base font-normal text-gray-500'>{repo.description? repo.description.slice(0,100) : "No Description Provided"}</p>
			{PROGRAMMING_LANGUAGES[repo.language] ? (
				<img src={PROGRAMMING_LANGUAGES[repo.language]} alt={repo.language} className='h-8' />
			) : null}
		</li>
	);
};
